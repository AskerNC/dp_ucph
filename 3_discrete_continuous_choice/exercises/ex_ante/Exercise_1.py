...
<<<<<<< HEAD
    for t in range(par.T-1, -1, -1):  #from last period until period 0, backwards 
        ...
        if t<par.T-1:
            # Precalculate interpretors before looping
...
<<<<<<< HEAD
    ...
    for t in range(par.T):
        ...
        # Fill in 
        #expV1 = np.exp(V1/par.sigma)
        #prob = expV1 / (expV1 + np.exp(V0/par.sigma))
        # Smoother formulation:
        prob = 1/(1+np.exp((V0-V1)/par.sigma)) 
