...
@with_kw mutable struct deaton_model
    ...
    #spaces 
    xbar::Array=[eps(0.0), 10.] # Upper and lower bound on cash on hand
    ...
    c::Array = initiate_c(n_x,n_c,xbar)
...
function initiate_c(n_x,n_c,xbar)
    c = Array{Float64}(undef,(n_x,n_c))
    
    for i in 1:1:n_x
        c[i,:] = LinRange(xbar[1],xbar[2],n_c)
    end
    return c
end
...
"""
function bellman(dm::deaton_mode,V0::Array)
    "Bellman operator, V0 is one-dim vector of values on state grid"
    interp = LinearInterpolation(dm.x,V0)
    EV=0
