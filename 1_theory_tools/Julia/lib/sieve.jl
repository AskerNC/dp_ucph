...
function plot1d(;p=plot(),f=nothing,x0=nothing,fx0=nothing,fhat=nothing,a=nothing,b=nothing,label="",color=:blue)
    ...
    x = LinRange(a,b,1000)
    if fx0 !== nothing
        plot!(p,x0,fx0,color=:red,seriestype = :scatter,label="") #Interpolation data
    end
    if f !== nothing
        plot!(p,x, f.(x), color=:grey, label = "f(x)")
    end
    if fhat!== nothing
        plot!(p,x,fhat.(x),color=color,label=label)
    end
    if (f!==nothing) | (fhat !== nothing)
        plot!(p,legend=true)
    end
end
...
function plot2d(x1,x2,f,p=nothing, label =L"f\left(x_{1},x_{2}\right)")
    if p === nothing
        p = plot()
    end
    plot!(p,x1,x2,f,st=:surface,xlabel=L"x_{1}",ylabel= L"x_{2}",title=label)
    return p
end



@with_kw mutable struct sieve_struct
    n              
    deg            
    a   
    b   
...
function make_sieve_struct(; n=6, deg=nothing, a=0, b=1, btype="chebyshev", gridtype="c", nknots=nothing,
                transform =nothing, inv_transform =nothing)

    s = sieve_struct(n,deg,a,b,btype,gridtype,nknots,transform,inv_transform,nothing)


    if isa(s.n,Array)==false
        s.n = Array([s.n])
    end

    if s.deg === nothing; s.deg = s.n.-1 end
    if s.nknots === nothing; s.nknots = s.n.+1 .-maximum(s.deg) end
    s.k = length(s.n)
    
    for x in [:deg,:a,:b,:btype,:gridtype,:nknots]
        setfield!(s,x,expand(getfield(s,x),s.k))
    end
